...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($cuser) {
            $jobs = $cuser->is('customer') 
                ? $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback')
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser) {
            if ($cuser->is('customer')) {
                ...
                return ['emergencyJobs' => $emergencyJobs, 'normalJobs' => [], 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => 'customer'];
            } 

            if ($cuser->is('translator')) {
                $jobs = Job::getTranslatorJobsHistoric($cuser->id, 'historic', $page);
                $numpages = ceil($jobs->total() / 15);
                return ['emergencyJobs' => $emergencyJobs, 'normalJobs' => $jobs, 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => 'translator', 'numpages' => $numpages, 'pagenum' => $page];
            }
        }
    }
    ...
    public function store($user, $data)
    {
        ...
        $data['due'] = $data['immediate'] === 'yes' 
            ? Carbon::now()->addMinutes(5)->format('Y-m-d H:i:s')
            : Carbon::createFromFormat('m/d/Y H:i', $data['due_date'] . ' ' . $data['due_time'])->format('Y-m-d H:i:s');
        
        $data['b_created_at'] = now();
        $data['will_expire_at'] = isset($data['due']) ? TeHelper::willExpireAt($data['due'], $data['b_created_at']) : null;
        $data['job_for'] = $this->formatJobFor($data['job_for']);
    ...
    public function storeJobEmail($data)
    {
        ...
        $job->fill([
            ...
            'address' => $data['address'] ?? $job->user->userMeta->address,
            'instructions' => $data['instructions'] ?? $job->user->userMeta->instructions,
            'town' => $data['town'] ?? $job->user->userMeta->city,
        ])->save();

        $email = $job->user_email ?? $job->user->email;
        $this->mailer->send($email, $job->user->name, 'Vi har mottagit er tolkbokning. Bokningsnr: #' . $job->id, 'emails.job-created', ['user' => $job->user, 'job' => $job]);
    ...
    private function formatJobFor($jobFor)
    {
        if (in_array('male', $jobFor)) return 'male';
        if (in_array('female', $jobFor)) return 'female';
        if (in_array('certified', $jobFor)) return 'certified';
